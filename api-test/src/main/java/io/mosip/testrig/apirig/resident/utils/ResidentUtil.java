...
public class ResidentUtil extends AdminTestUtil {
	...
	public static String getPdfPassword() {
	    ...
	    String fullNameJson = null;
	    String dob = null;

	    for (String key : autoGeneratedIDValueCache.keySet()) {

	        if (key.endsWith("_identity_fullName")) {
	            fullNameJson = autoGeneratedIDValueCache.get(key);
	        }

	        if (key.endsWith("_identity_dateOfBirth")) {
	            dob = autoGeneratedIDValueCache.get(key);
	        }
	    }

	    if (fullNameJson == null || dob == null) {
	        throw new RuntimeException("Required identity fields not found in cache");
	    }

	    // Parse fullname JSON array
	    JSONArray fullNameArray = new JSONArray(fullNameJson);
