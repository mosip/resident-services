...
public class SimplePost extends ResidentUtil implements ITest {
	...
	public void test(TestCaseDTO testCaseDTO) throws AuthenticationTestException, AdminTestException, SecurityXSSException {
		...
		String emailIdKey = null;
		if (testCaseName.contains("_SendOtpForExpiration_")) {
			JSONObject request = new JSONObject(inputJson);
			if (request.has("email") && !request.isNull("email")) {
				emailIdKey = request.get("email").toString();
				request.remove("email");
				inputJson = request.toString();
			} else {
				logger.warn("Email field is missing or null for test case: " + testCaseName);
			}
		}
		...
		if (testCaseDTO.getTemplateFields() != null && templateFields.length > 0) {
			...
			if (testCaseName.contains("_SendOtpForExpiration_")) {
				if (emailIdKey == null) {
					logger.error("Email ID key is null for test case: " + testCaseName);
					throw new AdminTestException("Email ID key not found for SendOtpForExpiration test");
				}
				String emailId = replaceIdWithAutogeneratedId(emailIdKey, "$ID:");
				...
				writeAutoGeneratedId(testCaseName, "OTP", NotificationListener.getOtp(emailId));
			}
		}
