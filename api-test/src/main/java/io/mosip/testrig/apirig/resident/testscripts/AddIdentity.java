...
public class AddIdentity extends ResidentUtil implements ITest {
	...
	public void test(TestCaseDTO testCaseDTO) throws AuthenticationTestException, AdminTestException, SecurityXSSException {
		...
		JSONObject identityObj = requestObj.getJSONObject("request").getJSONObject("identity");

		if (identityObj.has("dateOfBirth")) {
			String dobValue = identityObj.getString("dateOfBirth");
			writeAutoGeneratedId(testCaseDTO.getTestCaseName(), "DOB", dobValue);
		}

		if (identityObj.has("fullName")) {
			String fullNameValue = identityObj.getJSONArray("fullName").getJSONObject(0)
					.getString("value");

			writeAutoGeneratedId(testCaseDTO.getTestCaseName(), "FULLNAME", fullNameValue);
		}
	}
