...
public class IdSchemaUtilTest {
    ...
    public void loadDefaultFieldsTest() throws Exception {
        ...
        when(restClientService.getApi(ApiName.MIDSCHEMAURL, null, Lists.newArrayList(SCHEMA_VERSION_QUERY_PARAM), Lists.newArrayList(schemaVersion), String.class)).thenReturn(responseJson);
        ...
        verify(restClientService, times(1)).getApi(ApiName.MIDSCHEMAURL, null, Lists.newArrayList(SCHEMA_VERSION_QUERY_PARAM), Lists.newArrayList(schemaVersion), String.class);
    }

    @Test(expected = ApisResourceAccessException.class)
    public void throwApiResourceAccessIfResponseIsNull() throws Exception {
        Double schemaVersion = 0.1d;
        when(restClientService.getApi(ApiName.MIDSCHEMAURL, null, Lists.newArrayList(SCHEMA_VERSION_QUERY_PARAM), Lists.newArrayList(schemaVersion), String.class)).thenReturn(null);
        idSchemaUtil.loadDefaultFields(schemaVersion);
    }

    @Test
    public void testIfIdSchemaMapIsUsedAndRestClientNeverInvoked() throws JSONException, ApisResourceAccessException, IOException {
        Double schemaVersion = 0.1d;
        ...
        idSchemaUtil.loadDefaultFields(schemaVersion);
        verify(restClientService, never()).getApi(ApiName.MIDSCHEMAURL, null, Lists.newArrayList(SCHEMA_VERSION_QUERY_PARAM), Lists.newArrayList(schemaVersion), String.class);
    }
