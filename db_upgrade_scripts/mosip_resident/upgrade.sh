...
if [ $ACTION == "upgrade" ]; then
  ...
  if [ -f "$REVOKE_SCRIPT_FILE" ]; then
    ...
    PGPASSWORD=$SU_USER_PWD psql -v ON_ERROR_STOP=1 -v dbuserpwd="'${SU_USER_PWD}'" --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -a -b -f $REVOKE_SCRIPT_FILE
  else
